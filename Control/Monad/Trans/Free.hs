...
raiseT :: (Functor f, Monad m) => f a -> FreeT Either f m a
raiseT = FreeT . pure . Join . Biff . Right . Compose . fmap pure
...
coraiseT :: (Functor f, Comonad ɯ) => FreeT (,) f ɯ a -> f a
coraiseT = fmap copure . getCompose . snd . unBiff . unJoin . copure . freeT
